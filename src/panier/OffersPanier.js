...
import "./panier.css"
export default function OffersPanier() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const res = await axios.get('http://127.0.0.1:8000/api/AuthinscriptionOffers');
                setData(res.data);
            } catch (error) {
                console.log(error);
            }
                    ...
                    <div className='overflow-x-auto'>
                        <table className='w-full text-left border border-red-600 rounded-lg'>
                            <thead className='bg-black bg-opacity-70 border-b-2 border-red-600 rounded-t-lg'>
                                <tr>
                                    <th className='py-4 text-sm font-semibold text-center text-white'>Offer Name</th>
                                    <th className='py-4 text-sm font-semibold text-center text-white'>Start date</th>
                                    <th className='py-4 text-sm font-semibold text-center text-white'>deadline</th>
                            ...
                            <tbody className='bg-black bg-opacity-70'>
                                {data.length > 0 ? data.map((e, ind) => (
                                    <tr key={ind} className='border-b last:border-b-0 border-red-600'>
                                        <td className='py-4 text-center text-white'>{e.nom_offre}</td>
                                        <td className='py-4 text-center text-white'>{e.date_offre_deb} </td>
                                        <td className='py-4 text-center text-white'>{e.date_offre_fin} </td>
