...
import "./panier.css"
export default function EventsPanier() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const res = await axios.get('http://127.0.0.1:8000/api/UserEvent');
                setData(res.data);
            } catch (error) {
                console.log(error);
            }
                    ...
                    <div className='overflow-x-auto'>
                        <table className='w-full text-left border border-red-600 rounded-lg'>
                            <thead className='bg-black bg-opacity-70 border-b-2 border-red-600 rounded-t-lg'>
                                <tr>
                                    <th className='py-4 text-sm font-semibold text-center text-white'>Event Name</th>
                                    <th className='py-4 text-sm font-semibold text-center text-white'>Event Date</th>
                                    <th className='py-4 text-sm font-semibold text-center text-white'>Paiement</th>
                            ...
                            <tbody className='bg-black bg-opacity-70'>
                                {data.length > 0 ? data.map((e, ind) => (
                                    <tr key={ind} className='border-b last:border-b-0 border-red-600'>
                                        <td className='py-4 text-center text-white'>{e.events.nom_event}</td>
                                        <td className='py-4 text-center text-white'>{e.events.date_event}</td>
                                        <td className='py-4 text-center text-white'>{e.paiement}</td>
