...
import cross from "../fitness/cross.png"
...
export default function Crossfit() {
  ...
  const response = async () => {
    try {
      ...
      const res = await axios.get(`http://127.0.0.1:8000/api/specialite/${4}`)
      setdataf(res.data)
    }
    catch (error) {
      console.log(error)
  ...
  return (
    <ScrollAnimation>
      <div className="relative text-white">
        <img src={cross} className="w-full h-screen" />
              ...
              {activeStep === steps.length ? (
                ...
                <React.Fragment>
                  <Typography sx={{ mt: 2, mb: 1 }} className="heigthp">
                    ...
                    </div> : activeStep == 1 ? <div className="w-full h-full flex flex-col space-y-3 items-center my-3 justify-around">
