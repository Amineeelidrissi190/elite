...
export default function EditGalerie() {
  const { id } = useParams();
  ...
  const handleSubmit = async (e) => {
      ...
      try {
          const res = await axios.post(`http://127.0.0.1:8000/api/gallerie/${id}`, formdata);
          Swal.fire({
              icon: 'success',
              title: 'Success',
    ...
    <div className="w-full md:mx-4 mx-0">
      <form onSubmit={handleSubmit} className="flex flex-col items-center justify-center w-full p-2 space-y-2 rounded-lg">
        <div className="bg-slate-950 w-full flex items-center justify-between px-5 py-3 rounded-lg">
          <h1 className="text-sm">Edit Gallery</h1>
          <input type="submit" value="Update" className="text-green-600 hover:underline py-2 cursor-pointer" />
        </div>
