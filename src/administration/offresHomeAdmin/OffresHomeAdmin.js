...
import ChartOffers from './ChartOffers';
import axios from 'axios';
import { useEffect } from 'react';
export default function OffresHomeAdmin() {
    ...
    const getinscriptionLast = async () => {
        try {
            ...
            const res = await axios.get("http://127.0.0.1:8000/api/LastsOffers");
            const res2 = await axios.get("http://127.0.0.1:8000/api/TotalinsOffres");
            settotal(res2.data);
            setoffres(res.data);
            setLoading(false)
        } catch (error) {
            ...
            <div className='w-full px-2 space-y-2'>
                ...
                <div className='w-full flex flex-col items-center justify-center rounded-lg space-y-1'>
                    <div className='w-full flex items-center justify-between rounded-lg bg-slate-950 px-3 py-3 text-sm'>
                        <p>Last offres inscriptions</p>
                        <Link to="/Afficherinscriptionoffr" className='hover:underline'>View all</Link>
                    </div>
                        ...
                        <table className='bg-slate-950 rounded-lg w-full text-black'>
                            <thead className="bg-slate-800 border-b rounded-lg whitespace-nowrap text-center border-black">
                                <tr className='w-full rounded-lg'>
                                    ...
                                    <th className="md:w-1/6 min-w-[120px] font-normal text-white ">offre  deadline</th>
                                    <th className="md:w-1/6 min-w-[120px] font-normal text-white ">Delete</th>
                                </tr>
                            </thead>
                            <tbody className='rounded-lg'>
                                {offres && offres.map((e, ind) => {
                                    return (
                                        <tr key={ind} className='text-center text-white text-sm'>
                                            ...
                                            <td className="md:w-1/6 min-w-[120px] font-normal">{e.offre ? e.offre.date_offre_fin : 'Not available'}</td>
                                            <td className="md:w-1/6 min-w-[120px] font-normal"><Link to={`/Deleteinsoffer/${e.id}`} className="text-red-600 hover:underline">Delete</Link></td>
                                        </tr>
                                    );
                                })}
                            </tbody>
